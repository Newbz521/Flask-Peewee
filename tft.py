...
def champs(name=None):
  ...
  if request.method == "PUT":
      ...
      Champion.update(req).where(Champion.name == name).execute()
      return jsonify(model_to_dict(Champion.get(Champion.name == name))) 
  ...
  if request.method == 'DELETE':
    return jsonify(Champion.get(Champion.name == name).delete_instance())
...
def items(name=None):
  if request.method == 'GET':
    if name:
      return jsonify(model_to_dict(Item.get(Item.name == name)))
    else:
      itemList = []
  ...
  if request.method == "PUT":
      req = request.get_json()
      Item.update(req).where(Item.name == name).execute()
      return jsonify(model_to_dict(Item.get(Item.name == name)))
  if request.method == 'POST':
    ...
    return jsonify({"success": True})
  if request.method == 'DELETE':
    return jsonify(Item.get(Item.name == name).delete_instance())
